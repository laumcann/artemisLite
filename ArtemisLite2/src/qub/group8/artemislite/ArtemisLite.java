...
public class ArtemisLite {
	...
	/**
	 * int constant variable which will represent the number of sides that the Dice
	 * object has in the game.
	 */
	private static final int NUM_OF_DICE_SIDES = 6;

	/**
	 * int constant variable which will represent the number of Squares that the
	 * virtual game board will have.
	 */
	...
	public void playGame(Scanner scanner) {
		...
		dice = new Dice(NUM_OF_DICE_SIDES);
		...
		for (int playerNum = 1; playerNum <= numOfPlayers; playerNum++) {
			...
			do {
				...
				if (playerName.isEmpty()) {
					System.out.println("The Cadet name can not be blank. Please enter a name to continue.");
					...
					/*
					 * Else if part of if statement checks if a numeric value has been provided.
					 * Uses the isNumeric method of the ArtemisLite class.
					 */
				} else if (isNumeric(playerName)) {
					System.out.println(
							"\nThe Cadet name can not be a number. Please enter a valid name " + "to continue.");
	...
	/**
	 * Method that will be used to assess if a numeric value has been provided as a
	 * Player name.
	 * 
	 * @param strNum - String that represents the name that has been provided for a
	 *               Player.
	 * @return - Returns true if a numeric value has been provided as a Player name.
	 *         Returns false if a non-numeric value has been provided as a Player
	 *         name.
	 */
	public boolean isNumeric(String playerName) {

		/*
		 * Try-catch block to try and parse playerName to an int. If it can't be parsed,
	...
	public void turnManager(List<Player> playerList, Dice dice, Scanner scanner) {
		...
		/*
		 * boolean variable that will be used to check if all Elements have been fully
		 * developed. Initially set as false.
		 */
		...
		do {
			...
			try {
				...
				if ((playerList.size() == 1) && (currentPosition == 7)) {
					 ...
					 * not shown.
					 */
				} else if ((hasRolled) && (currentPosition == 7)) {
				...
				case 1:
					...
					if (!hasRolled) {
						...
						/*
						 * If all Elements on the board have been fully developed, allElementsDeveloped
						 * becomes true.
						 */
						if (checkAllDeveloped(board) == 30) {
							allElementsDeveloped = true;
						}
		...
		} while ((!quitGame) && (!playerBankrupt) & (!allElementsDeveloped));
		...
		if (quitGame) {
			 ...
			 * bankrupt.
			 */
		} else if (playerBankrupt) {
			MessageCentre.displayPlayerBankruptMessage(whoIsBankrupt);
		}

		/*
		 * Static method of MessageCentre class that displays a summary of the game play
		 * to screen.
		 */
		MessageCentre.displayGamePlayResultsToScreen(playerList, quitGame, playerBankrupt, allElementsDeveloped);
	}
	...
	public int newPlayerPosition(int numRolled, int currentPlayerPosition) {
		...
		/*
		 * Checks if numRolled plus the current Player's position is greater than the
		 * value of the BOARD_SIZE constant. If so, the Player will have passed the
		 * starting Square on the board, and the value of the BOARD_SIZE constant is
		 * subtracted from the current Player's position to account for this.
		 */
		if (currentPlayerPosition + numRolled > BOARD_SIZE) {
			return (currentPlayerPosition - BOARD_SIZE) + numRolled;
		} else {
		...
		if (currentPlayerPosition == 1) {
			...
			if (currentPlayerPosition != 7) {
				...
				if (elementOwner.equals("Unoccupied")) {
					...
					/* Displays the details of the Element that the current Player is on. */
					element.displayDevelopmentDetails();
					...
					if (!checkFullSystemOwned(currentPlayer, element.getSystemType())) {
						...
						/*
						 * Checks if the Element has been developed to level 3. If it has, the option to
						 * develop the Element will not be displayed.
						 */
					} else if (element.getDevelopmentLevel() == 3) {

						System.out.println("\n\n|*****| Element Fully Developed |*****|");

						System.out.println("\nThis Element has been developed to Major Development Status. This "
			...
			try {
				...
				case 1:
					...
					if (playerList.indexOf(currentPlayer) + 1 == 1) {
						...
						/* confirmGiftElement method of ArtemisLite class is called. */
						confirmGiftElement(currentPlayer, receivingPlayer, element, scanner);
	 ...
	 *                        entries.
	 */
	public void confirmGiftElement(Player currentPlayer, Player receivingPlayer, Element element, Scanner scanner) {
		...
		do {
			try {
				...
				case 1:
					...
					/*
					 * Static method of MessageCentre class which displays a message confirming that
					 * the currentPlayer has gifted the Element to the receivingPlayer.
					 */
					MessageCentre.displayElementGiftedMessage(currentPlayer.getPlayerName(), element.getSquareName(),
		...
		if (totalElementsOwned == 0) {
			...
			do {
				try {
					...
					/*
					 * Static method of MessageCentre class that displays a message before the
					 * Elements that Player owns are displayed.
					 */
					MessageCentre.displayElementsOwnedMessage(currentPlayerName);
					...
					case 1:
						...
						if (developmentCost > currentPlayer.getPlayerFunds()) {
							...
							if (!checkFullSystemOwned(currentPlayer, element.getSystemType())) {
								...
								/*
								 * Static method of MessageCentre class which displays a message if a Player
								 * tries to develop an Element when they don't own it's full System.
								 */
								MessageCentre.displayDevelopElementUnavailableMessage(element.getSystemType());
								break;
							}
	 ...
	 *         board.
	 */
	public ArrayList<Square> initialiseBoard() throws IllegalArgumentException {

		/* Creates a new ArrayList to store Square objects. */
		ArrayList<Square> board = new ArrayList<Square>(BOARD_SIZE);
		...
		String square4Description = "Your Crawler-Transporter will transport your Mobile Launcher Platform\nand "
				+ "Spacecraft to this piece of ground at John F. Kennedy Space Center for launch";
		...
		String square6Description = "Astronauts will learn how to operate the Orion Multi-Purpose Crew Vehicle\n"
				+ "(Orion MPCV) by training in a realistic simulator";
		String square7Description = "No work activity will be conducted when present on this square";
		String square8Description = "This is the state of the art software system that will support the\nentire "
				+ "Artemis mission";
		...
		String square11Description = "This is the exploration vehicle that will carry the crew to space and to "
				+ "the moon";
		String square12Description = "This is the launch vehicle that will launch the Orion Multi-Purpose "
		...
		String square6DevLevel3Message = "Orion Spacecraft Simulator Training fully developed to Level 3.\n\nThe "
				+ "interior of the simulator has been outfitted with Orion's display and control system and\ncrew "
				+ "seats and is ready for astronaut training.";
		...
		String square10DevLevel2Message = "Space Communications and Navigation developed to Level 2.\n\nThe Tracking "
				+ "and Data Relay Satellites have been developed, allowing communications with Artemis\nduring "
				+ "low-Earth orbit\nto be achieved.";
		...
		try {
			 ...
			 */
			Payroll square1 = new Payroll(SquareName.PAYROLL, square1Description, PAYROLL_BONUS);
			 ...
			 * system.
			 */
			Element square2 = new Element(SquareName.CRAWLER_TRANSPORTER, square2Description, SystemType.LAUNCH_COMPLEX,
					null, STANDARD_PURCHASE_COST, STANDARD_DEVELOPMENT_COST, square2DevLevel1Message,
					square2DevLevel2Message, square2DevLevel3Message);
			 ...
			 * system.
			 */
			Element square3 = new Element(SquareName.MOBILE_LAUNCHER_PLATFORM, square3Description,
					SystemType.LAUNCH_COMPLEX, null, STANDARD_PURCHASE_COST, STANDARD_DEVELOPMENT_COST,
			 ...
			 */
			Element square4 = new Element(SquareName.LAUNCH_PAD, square4Description, SystemType.LAUNCH_COMPLEX, null,
					STANDARD_PURCHASE_COST, STANDARD_DEVELOPMENT_COST, square4DevLevel1Message, square4DevLevel2Message,
					square4DevLevel3Message);
			 ...
			 */
			Element square5 = new Element(SquareName.NEUTRAL_BUOYANCY_LABORATORY_TRAINING, square5Description,
					SystemType.TRAINING_PROGRAM, null, MIN_PURCHASE_COST, MIN_DEVELOPMENT_COST, square5DevLevel1Message,
					square5DevLevel2Message, square5DevLevel3Message);
			 ...
			 */
			Element square6 = new Element(SquareName.ORION_SPACECRAFT_SIMULATOR_TRAINING, square6Description,
					SystemType.TRAINING_PROGRAM, null, MIN_PURCHASE_COST, MIN_DEVELOPMENT_COST, square6DevLevel1Message,
					square6DevLevel2Message, square6DevLevel3Message);
			 ...
			 */
			UnpaidLeave square7 = new UnpaidLeave(SquareName.UNPAID_LEAVE, square7Description);
			 ...
			 * a 3 Element system.
			 */
			Element square8 = new Element(SquareName.SPACEPORT_COMMAND_AND_CONTROL_SYSTEM, square8Description,
					SystemType.COMMAND_AND_CONTROL, null, STANDARD_PURCHASE_COST, STANDARD_DEVELOPMENT_COST,
			 ...
			 * system.
			 */
			Element square9 = new Element(SquareName.MISSION_CONTROL_CENTRE, square9Description,
					SystemType.COMMAND_AND_CONTROL, null, STANDARD_PURCHASE_COST, STANDARD_DEVELOPMENT_COST,
			 ...
			 * a 3 Element system.
			 */
			Element square10 = new Element(SquareName.SPACE_COMMUNICATIONS_AND_NAVIGATION, square10Description,
					SystemType.COMMAND_AND_CONTROL, null, STANDARD_PURCHASE_COST, STANDARD_DEVELOPMENT_COST,
			 ...
			 */
			Element square11 = new Element(SquareName.ORION_MULTIPURPOSE_CREW_VEHICLE, square11Description,
					SystemType.SPACECRAFT, null, MAX_PURCHASE_COST, MAX_DEVELOPMENT_COST, square11DevLevel1Message,
					square11DevLevel2Message, square11DevLevel3Message);
			 ...
			 */
			Element square12 = new Element(SquareName.SPACE_LAUNCH_SYSTEM, square12Description, SystemType.SPACECRAFT,
					null, MAX_PURCHASE_COST, MAX_DEVELOPMENT_COST, square12DevLevel1Message, square12DevLevel2Message,
					square12DevLevel3Message);
		...
		/*
		 * Returns the created virtual game board of Square / Element objects. If the
		 * number of Squares / Elements added to the board ArrayList is not equal to the
		 * BOARD_SIZE constant variable, an IllegalArgumentException is thrown.
		 */
		if (board.size() > BOARD_SIZE || board.size() < BOARD_SIZE) {
			throw new IllegalArgumentException("Invalid number of Squares / Elements provided to board variable. The "
					+ "number of Squares must be equal to " + BOARD_SIZE + ".");
		} else {
			return board;
		}
	}
		...
		do {
			try {
				...
				case 1:
					...
					int taxAmount = element.getTaxCost();
	...
	/**
	 * Method that checks if all Elements on the virtual game board have been fully
	 * developed.
	 * 
	 * @param board - ArrayList of Square objects which represents the virtual game
	 *              board.
	 * @return - Returns the total of all the development levels of Elements on the
	 *         virtual game board. If the total equals 30, all Elements have been
	 *         fully developed.
	 */
	public int checkAllDeveloped(List<Square> board) {

		/* int variable to store the total of all development levels. */
		int total = 0;

		/* Loops through each Square on the virtual game board. */
		for (Square square : board) {

			/* Checks that the Square is an Element. */
			if (Element.class.isInstance(square)) {

				/* Gets the development level and adds it to total. */
				total += ((Element) square).getDevelopmentLevel();
			}
		}
		return total;
	}
